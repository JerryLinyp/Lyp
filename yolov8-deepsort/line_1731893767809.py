...
def refine_detections(detections, scale_factor=0.9):
    """调整检测框大小。"""
    refined_detections = np.copy(detections)
    for i in range(len(refined_detections)):
        x1, y1, x2, y2 = refined_detections[i]
        w, h = x2 - x1, y2 - y1
        x1 += (1 - scale_factor) * w / 2
        y1 += (1 - scale_factor) * h / 2
        x2 -= (1 - scale_factor) * w / 2
        y2 -= (1 - scale_factor) * h / 2
        refined_detections[i] = [int(x1), int(y1), int(x2), int(y2)]
...
def detect_and_track(input_path: str, output_path: str, detect_class: int, model, tracker, lines) -> Path:
    ...
    while True:
        ...
        results = model(frame, conf=0.5, iou=0.4, stream=True)
